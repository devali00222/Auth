...
export const usernameExistInDatabase: CustomValidator = async (
  ...
  const user: UserData | null = await UserModel.findOne<UserData>({ username });
  if (user) {
    throw new Error("this user is already being used");
  }
  return true;
};
...
export const emailIsRegisteredInDatabase: CustomValidator = async (
  ...
  const user: UserData | null = await UserModel.findOne<UserData>({ email });
  if (!user) {
